...
outlier_metric <- function(fit, outlier_method, outlier_quantile, count_space) {
  ...
  if (outlier_method == "var.e") {
    ...
    metric[is.na(metric) | is.nan(metric)] <- 0
...
deconv <- function(test, cellmat, comp_amount, m_itself) {
  ...
  if (rcond(m_itself) < .Machine$double.eps) {
    warning("Spillover matrix is near-singular, results may be unstable")
  }
  rawcomp <- solve(m_itself)
  # Compensation: blend between full compensation and no compensation
  blend <- comp_amount * diag(nrow(m_itself)) + (1 - comp_amount) * t(m_itself)
  mixcomp <- solve(m_itself, t(blend))
  output <- dotprod(test, cellmat) %*% mixcomp
  rs <- rowSums(output)
  rs[rs == 0] <- .Machine$double.eps
  percent <- output / rs * 100
